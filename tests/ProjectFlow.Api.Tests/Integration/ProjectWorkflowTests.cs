...
public sealed class ProjectWorkflowTests(ApiFactory factory) : IClassFixture<ApiFactory>
{
    ...
    public async Task AuthenticatedUser_CanCreateProjectWorkItemAndComment()
    {
        ...
        var project = await createProject.Content.ReadFromJsonAsync<ProjectResponse>(JsonOptions);
        ...
        var item = await createItem.Content.ReadFromJsonAsync<WorkItemResponse>(JsonOptions);
        Assert.NotNull(item);
        ...
        var list = await client.GetFromJsonAsync<WorkItemResponse[]>($"/api/projects/{project.Id}/work-items", JsonOptions);
        Assert.Single(list!);
